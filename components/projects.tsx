          ...
          <AnimatePresence mode="popLayout">
            {filtered.map((project, i) => (
              <motion.div
                ...
                className="group relative flex flex-col overflow-hidden rounded-3xl border border-[--color-border] bg-[--color-surface]/30 backdrop-blur-sm transition-colors duration-500 hover:border-[--color-border-strong]"
              >
                <div
                  aria-hidden
                  className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-700 group-hover:opacity-100"
                  style={{
                    background: `radial-gradient(circle at 50% 0%, ${accentMap[project.accent]}15, transparent 70%)`,
                  }}
                />

                <div className="relative aspect-[16/10] overflow-hidden border-b border-[--color-border] bg-[--color-bg-elev]">
                  <ProjectVisual project={project} hovered={hovered === project.id} />
                </div>
                ...
                <div className="relative flex flex-1 flex-col p-7">
                  <div className="flex flex-wrap items-center gap-2 text-[10px] font-mono uppercase tracking-[0.15em]">
                    <span
                      ...
                      style={{
                        borderColor: `${accentMap[project.accent]}40`,
                        color: accentMap[project.accent],
                      }}
                    >
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {project.stack.map((tech) => (
                      <span
                        key={tech}
                        className="rounded-md border border-[--color-border] bg-[--color-bg-elev]/60 px-2 py-1 font-mono text-[10px] text-[--color-fg-muted]"
                      >
                        {tech}
                      </span>
                    ))}
                      ...
                      {project.highlights.map((h) => (
                        <li key={h} className="flex gap-2">
                          <span
                            ...
                            style={{ background: accentMap[project.accent] }}
                          />
                          {h}
                        </li>
                      ))}
